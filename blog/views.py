...
def home(request):
    ...
    # t_categories = Category.objects.filter(id=id, name=name)
...
def post_detail(request, id):
    ...
    if request.method == 'POST':
        ...
        if comments_form.is_valid():
            body = comments_form.cleaned_data['body']
            parent_id = comments_form.cleaned_data['parent']
            user = request.user
            if parent_id is not None:
                try:
                    parent_comment = Comment.objects.get(id=parent_id)
                    new_comment = Comment(
                        body=body, user=user, post=post, parent=parent_comment)
                except Comment.DoesNotExist:
                    pass
            else:
                new_comment = Comment(
                    body=body, user=user, post=post, parent=None
                )
            new_comment.save()
